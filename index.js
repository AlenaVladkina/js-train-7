...
function createSymbolWithDescription(property) {
  ...
  return sym.description;//Повертаємо опис символа symbol
}
...
function createSymbolProperty(property) {
  const sym = Symbol();// Створимо змінну sym яка дорівнює символу без опису
  const obj = {};// Створимо пустий об'єкт
  obj[sym] = property;// Додамо властивість до об'єкту, ключем якої буде наш символ, а значенням буде аргумент property
...
function createAndCompareTwoSymbols(value) {
  ...
  return sym1 === sym2;// Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
}
...
function createSymbolForKey(key) {
  const symbol = Symbol.for(key);// Використовуємо Symbol.for() для створення нового символу symbol зі значенням key
  ...
  return symbol;// Повертаємо створений символ
}
...
function compareSymbolsForKey(symbol1, symbol2) {
  if(symbol1 === symbol2){
    ...
    return false};// Використовуємо оператор '===' для порівняння двох символів
...
function useSymbolToStringTag() {
  const myObject = {};// Створюємо пустий об'єкт myObject.
  const mySymbol = Symbol();
  myObject[mySymbol] = "CustomObject";// Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
  return myObject[mySymbol].toString();// Повертаємо рядкове представлення myObject[Symbol].
}
...
function useSymbolDescription() {
  const mySymbol = Symbol("mySymbol");// Створюємо символ mySymbol зі значенням "mySymbol".
  const symbolDescription = mySymbol.description;// Отримуємо опис символу mySymbol в змінну symbolDescription за допомогою властивості description.
...
let myObject = {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        return currentIndex < this.category.length ? //Створюємо властивість currentIndex зі значенням 0, яка буде нашим лічильником в ітерації
        {value: this.category[currentIndex++].name, done: false } : {done: true};
      }
    };
  }
